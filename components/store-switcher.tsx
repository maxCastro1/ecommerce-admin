    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
           <PopoverTrigger asChild>
             <Button variant='outline' size='sm' role="combobox" arial-aria-expanded='open' aria-label="select a store" className={cn("w-[200px] justify-between" , className)}>
                ...
                <CommandGroup heading="Stores">
                  {formattedItems.map((store) => (
                    <CommandItem key={store.value} onSelect={()=> onStoreSelect(store)} className="text-sm">
                     <StoreIcon className="mr-2 h-4 w-4 cursor-pointer" />
                     {store.label}
